...
class DataPrefetcher(object):
    ...
    def preload(self):
        try:
            if len(self.list) == 0 or len(self.list) >= self.cnt:
                ...
                self.next_input, self.next_depth, self.next_target, self.next_boundary, self.next_gate = next(self.loader)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_boundary[idx] = self.next_boundary[idx].cuda(non_blocking=True)
    ...
    def next(self):
        ...
        boundary = self.next_boundary[self.idx]
        gate = self.next_gate[self.idx]
        self.preload()
        return input, depth, target, boundary, gate